...
log_path = "log/evaluation_log.txt"
        ...
        for i, (p, g) in enumerate(zip(plist, glist)):
            ...
            for idx, pg in enumerate(zip(p, g)):
                ...
                if etype in ["all", "exec"]:
                    ...
                    # Run SQL for log output
                    try:
                        conn = sqlite3.connect(db)
                        pred_exec_output = conn.execute(p_str).fetchall()
                    except Exception as e:
                        pred_exec_output = f"ERROR: {e}"

                    try:
                        conn = sqlite3.connect(db)
                        gold_exec_output = conn.execute(g_str).fetchall()
                    except Exception as e:
                        gold_exec_output = f"ERROR: {e}"
                ...
                # เขียน log
                f_log.write(f"""===== Eval Round {log_count} =====
Database         : {db_name}
Prediction SQL   : {p_str}
Gold SQL         : {g_str}
Exec Accuracy(EX): {exec_score}
Exact Match(EM)  : {exact_score}
Pred Output      : {pred_exec_output}
Gold Output      : {gold_exec_output}
